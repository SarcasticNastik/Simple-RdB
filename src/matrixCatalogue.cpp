...
void MatrixCatalogue::deleteMatrix(string matrixName)
{
    ...
    this->matrices[matrixName]->unload(); // SP: deleting temp files
    delete this->matrices[matrixName]; // SP: deleting address/memory
    this->matrices.erase(matrixName); // SP: Removing key
}
